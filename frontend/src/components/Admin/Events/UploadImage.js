...
class UplaodImage extends React.Component {
    ...
    onSubmit = async e => {
        e.preventDefault();
        this.props.setImagePath(`../../../../../backend/uploads/images/events/${this.state.filename}`);  //Path may change with the file structure

        const formData = new FormData();
        formData.append('event', this.state.file);

        try {
            const res = await axios.post('/upload/image/event', formData, {
                headers: {
